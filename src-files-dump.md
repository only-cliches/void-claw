...
```rs
/// Certificate Authority management for the agent-zero MITM proxy.
///
...
impl CaStore {
    ...
    fn build_ca_cert(key: &KeyPair) -> Result<rcgen::Certificate> {
        ...
        params
            .distinguished_name
            .push(DnType::CommonName, "agent-zero Proxy CA");
        params
            .distinguished_name
            .push(DnType::OrganizationName, "agent-zero");
...
fn prompt_config_creation_choice() -> Result<ConfigCreationChoice> {
    ...
    println!("2. Create default config at ~/.config/agent-zero/agent-zero.toml");
...
#!/usr/bin/env python3
"""
killme — agent-zero container exit command.

Requests that the agent-zero manager stop the current container session.
...
def main() -> None:
    ...
    token = os.environ.get("AGENT_ZERO_TOKEN", "")
    if not token:
        print("killme: AGENT_ZERO_TOKEN is not set", file=sys.stderr)
        sys.exit(1)

    session_token = os.environ.get("AGENT_ZERO_SESSION_TOKEN", "")
    if not session_token:
        print("killme: AGENT_ZERO_SESSION_TOKEN is not set", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"  Is agent-zero running? Is AGENT_ZERO_URL correct? ({base_url})", file=sys.stderr)
    sys.exit(1)
...
# agent-zero agent base — Ubuntu 24.04 LTS (Noble Numbat)
#
...
## agent-zero.example.toml
```t
# agent-zero.example.toml
#
# Main manager config for agent-zero.
# Per-project command rules, network policy, and agent instructions live in the
# project's own zero-rules.toml file, not here.
...
# `~/.config/agent-zero/docker`.
docker_dir = __AGENT_ZERO_DOCKER_DIR__
...
global_rules_file = "~/.config/agent-zero/rules.toml"

[workspace]
# Root directory for managed workspace copies.
# When a project does not set `workspace_path`, agent-zero uses:
...
# - exclude_patterns in the project's zero-rules.toml
...
token_env_var = "AGENT_ZERO_TOKEN"
...
# These are referenced by hostdo command rules in zero-rules.toml via
# `env_profile`.
# ---------------------------------------------------------------------------
...
# Add one [[projects]] block per canonical repository when you want Agent Zero
...
# If omitted, agent-zero uses /workspace.
...
image = "agent-zero-claude:ubuntu-24.04"

# Optional override for this profile's mount point.
# mount_target = "/workspace"

# Controls which agent bootstrap files agent-zero writes into the workspace.
...
image = "agent-zero-codex:ubuntu-24.04"
agent = "codex"
...
image = "agent-zero-gemini:ubuntu-24.04"
agent = "gemini"
...
image = "agent-zero-opencode:ubuntu-24.04"
agent = "opencode"
